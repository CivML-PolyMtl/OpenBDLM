...
Bayesian dynamic linear models are a class of linear gaussian state-space models which can be described from the transition and the observation equations.
...
\end{equation}
where, for each each time $t=1, \dots ,\mathtt{\text{T}}$, the variables $\mathbf{x}_{t}$ follow a Gaussian distribution with mean $\bm{\mu}_{t}$ and covariance matrix $\bm{\Sigma}_{t}$, $\mathbf{A}_{t}$ is the transition matrix, and $\mathbf{w}_{t}$ represents Gaussian model errors with zero mean and covariance matrix $\mathbf{Q}_{t}$.
...
In case of dependencies between the time series, regression coefficients are added in $\mathbf{C}_{t}$.
...
\label{EQ:KF}
\end{equation}
where $\mathcal{L}_{t}$ is the marginal likelihood describing the probability of observing observations $\mathbf{y}_{t}$ at time $t$ given all the observations up to time $t-1$.
The standard Kalman filter expressed in Eq.~\ref{EQ:KF} can process stationary, trend stationary, and acceleration stationary time series, but it is not capable of handling non-stationary time-series, which is a major limitation when it comes to anomaly detection.
...
One way to model changing dynamics is to run in parallel a collection of ${\mathtt{\text{S}}}$ linear models, each having their own system dynamics $\mathbf{A}_{t}$ and $ \mathbf{Q}_{t}$.
...
In such approach, a discrete markovian switching variable $s_{t}= 1, ..,j,.. ,\mathtt{\text{S}}$ with a transition probabilities matrix $\mathbf{Z}_{t}$ and probabilities $\bm{\pi}_{t}$ is introduced to indicate which dynamics is used at time $t$.
The problem of incorporating switching dynamics into the model is that the state vector grows in a way that the dimension of the state vector at time $t$ is $\mathtt{\text{S}}^{t}$.
...
This technique, known as the Switching Kalman filter, allows to keep the dimension of the state vector equal to $S$ at each time $t$.
...
$ \mathcal{L}_{t}^{i(j)}$  the marginal likelihood that describes the probability of observing observations $\mathbf{y}_{t}$ at time $t$ given all the observations up to time $t-1$, and given the state at time $t−1$ was $s_{t-1} = i$ and that it switches to $s_{t} = j$ at time $t$.
...
\begin{equation}
\pi_{t|t}^{j} = \sum_{i=1}^{\mathtt{\text{S}}} \frac{\mathcal{L}_{t}^{i(j)} \pi_{t-1|t-1}^{i} Z^{i(j)}_{t} }{c},
\label{EQ:StateProbability}
\end{equation}
where $c$ is a normalization constant ensuring that $ \sum_{j=1}^{\mathtt{\text{S}}} \pi_{t|t}^{j} = 1 $.
...
From Eq.~\ref{EQ:SKF2}, it is clear that the SKF algorithm provides a set a $\mathtt{\text{S}}$ state vectors at each time $t$.
...
The procedure of learning the model parameters is hereafter referred to as model parameters learning, or model identification.
...
\label{EQ:BT}
\end{equation} 
where $p(\mathbf{y}_{1:\mathtt{Tr}}|\bm\theta)$ is the likelihood PDF,  $p(\bm\theta)$ is the prior PDF.
...
\begin{equation}
\ln p(\mathbf{y}_{1:\mathtt{Tr}}|\bm\theta)  = \displaystyle\sum_{t=1}^{\mathtt{Tr}} \ln p(\mathbf{y}_{t}|\mathbf{y}_{1:t-1},\bm \theta) = \displaystyle\sum_{t=1}^{\mathtt{Tr}} \ln \left[ \sum_{j=1}^{\mathtt{\text{S}}} \sum_{i=1}^{\mathtt{\text{S}}} \mathcal{L}_{t}^{i(j)} \pi_{t-1|t-1}^{i} Z_{t}^{i(j)} \right] \text{,}
\label{EQ:LP}
\end{equation}
...
The Maximum log Likelihood Estimation (MLE) is a special case of the MAP where the prior PDF $p(\bm\theta)$ is assumed to be uniform.
...
The MAP and MLE are point estimation methods which do not take into account the uncertainty in the parameter estimates. 
The estimation of the uncertainties in the model parameters estimates can be addressed using the Laplace approximation.
\begin{gather}
\begin{aligned}
p(\bm\theta|\mathbf{y}_{1:\mathtt{Tr}}) & \approx  \mathcal{N}\left(\bm\theta;\bm\theta^{*},-\mathcal{H}(\bm\theta^{*})^{-1}\right) \text{for the MAP} \\
p(\mathbf{y}_{1:\mathtt{Tr}} | \bm\theta) & \approx  \mathcal{N}\left(\bm\theta;\bm\theta^{*},-\mathcal{H}(\bm\theta^{*})^{-1}\right) \text{for the MLE},
\label{EQ: LaA}
\end{aligned}
\end{gather}
where $\mathcal{H}(\bm\theta^{*})$ is the second derivative of the log a posteriori or log likelihood PDF evaluated at the optimal parameter values $\bm\theta^{*}$. 

\subsubsection{Parameter-wise Newton-Raphson}

The parameter-wise Newton-Raphson algorithm is an iterative approach which can be used to find the model parameters that correspond to the maximum of a target PDF, hereafter noted $\mathcal{L}_{1:\mathtt{Tr}}(\bm{\theta})$.
The function $\mathcal{L}_{1:\mathtt{Tr}}(\bm{\theta})$  is either the log a posteriori or the log likelihood PDF.
One iteration of the Newton-Raphson algorithm is
\begin{equation}
\bm{\theta}_{\text{new}}  = \bm{\theta}_{\text{old}} - \lambda \frac{\nabla \mathcal{L}_{1:\mathtt{Tr}}(\bm{\theta}_{\text{old}}^{i}) }{  \nabla^{2} \mathcal{L}_{1:\mathtt{Tr}}(\bm{\theta}_{\text{old}}^{i})},
\label{EQ:NR}
\end{equation}
...
The convergence of each model parameters is reached when
\begin{equation}
\left\{\begin{array}{ccc}
\mathcal{L}_{1:\mathtt{Tr}}(\bm\theta^{i}_{\text{old}}) &<&\mathcal{L}_{1:\mathtt{Tr}}(\bm\theta^{i}_{\text{new}})\\[4pt]
\left|\mathcal{L}_{1:\mathtt{Tr}}(\bm\theta^{i}_{\text{new}}) -  \mathcal{L}_{1:\mathtt{Tr}}(\bm\theta^{i}_{\text{old}})\right| &\leq& \tau \cdot \left|\mathcal{L}_{1:\mathtt{Tr}}(\bm\theta^{i}_{\text{old}})\right|
\end{array}\right.,
\label{EQ:STC}
\end{equation}
...
In most cases. the derivatives of $\mathcal{L}_{1:\mathtt{Tr}}(\bm{\theta})$ can not be computed analytically, and the derivatives are approximated numerically using the central differentiation scheme, such as
\begin{gather}
\begin{aligned}
 \nabla \mathcal{L}_{1:\mathtt{Tr}}(\bm\theta^{i}) & = \frac{\partial \mathcal{L}_{1:\mathtt{Tr}} (\bm\theta) }{\partial \theta^{i}} \approx \frac{\mathcal{L}_{1:\mathtt{Tr}} (\bm\theta + \Pi(i)\Delta \theta^{i} )  -  \mathcal{L}_{1:\mathtt{Tr}} (\bm\theta - \Pi(i)\Delta \theta^{i} ) }{2\Delta \theta^{i}}  \\
 \nabla^{2} \mathcal{L}_{1:\mathtt{Tr}}(\bm\theta^{i}) & = \frac{\partial^{2} \mathcal{L}_{1:\mathtt{Tr}} (\bm\theta) }{\partial^{2} \theta^{i}} \approx \frac{\mathcal{L}_{1:\mathtt{Tr}} (\bm\theta + \Pi(i)\Delta \theta^{i} )  -  2 \mathcal{L}_{1:\mathtt{Tr}} (\bm\theta) +  \mathcal{L}_{1:\mathtt{Tr}} (\bm\theta - \Pi(i)\Delta \theta^{i} ) }{(\Delta \theta^{i})^{2}},
...
%The standard deviation model parameters are real numbers that lie in the $[0, +\text{Inf}]$ interval.
%The autoregression coefficient model parameters are real numbers that lie in the $[0, 1]$ interval.
During the learning procedure, it may happen that new model parameters $\bm\theta_{\text{new}}$ are proposed outside their valid interval.
Those parameters must be rejected, which strongly hinders the computational efficiency of the learning algorithm.
One possibility is to transform the bounded parameters space into an unbounded parameters space, where the parameters lie in the $[ -\text{Inf}, +\text{Inf} ]$ interval, and to perform the learning procedure in the transformed, unbounded, space.
The transformation is done using a function $g(.)$ so that, 
\begin{equation}
\theta^{i, \text{tr}} = g(\theta^{i}) \text{, }\quad \theta^{i, \text{tr}} \in [-\text{Inf}, +\text{Inf} ] \text{.}
\end{equation}
The choice of the function $g(.)$ depends on the bound of $\theta$. 
Three cases generally occur:
\begin{itemize}
\item $ \theta \in [-\text{Inf}, +\text{Inf} ]$ , $g(\theta) = 1$, so that $\theta^{\text{tr}} = \theta$ and $\theta = \theta^{\text{tr}} $
\item $ \theta \in [0, +\text{Inf} ]$, $g(\theta) = \ln(\theta)$, so that $\theta^{\text{tr}} = \ln(\theta) $ and $\theta = e^{\theta^{\text{tr}}} $
...
\end{itemize}
For instance, the standard deviation model parameters are real numbers that lie in the $[0, +\text{Inf}]$ interval and the logarithme transformation is used.
Moroever, the autoregression coefficient model parameters are real numbers that lie in the $[0, 1]$ interval, and the sigmoid transformation is used.

\section{Block component}
\subsection{Local Level}
\subsection{Local Trend}
\subsection{Local Acceleration}
\subsection{Local Level Trend compatible}
\subsection{Local Level Acceleration compatible}
\subsection{Local Trend Acceleration compatible}
\subsection{Periodic}
\subsection{First order Autoregressive}
...
The \MATLAB{} binary .MAT file must contain three \MATLAB{} variables called \lstinline[basicstyle = \mlttfamily \bf \small]!labels!, \lstinline[basicstyle = \mlttfamily \small \bf]!timestamps!, and \lstinline[basicstyle = \mlttfamily \small \bf]!values!.
...
The field \lstinline[basicstyle = \mlttfamily \bf]!model.component.const! stores a $1\times \mathtt{C}$ cell array, where $\mathtt{C} = \{1, 2 \}$ is the total number of model classes.
It is defined only if $\mathtt{C} = 2$.
The first cell is empty, and the second cell is a $1\times \mathtt{M}$ cell array of array, where $\mathtt{M}$ is the number of time series.
The array contains $0$ and $1$ to indicate which block components of the second model class has the same model parameters than the corresponding component of the first model class. 
A value of $1$ indicates that the model parameters are constrained between the block components of the two model classes, $0$ otherwise.
