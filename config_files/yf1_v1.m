...
data.values{1}=yf1_semaine; %Enter your dataset here; each column is a dataset; missing data -> NaN 
data.timestamps{1}=t_yf1_semaine; %Enter the time stamps (Unix format); column vector; 
clear yf1_semaine t_yf1_semaine
data.labels={'ext'};

data.interventions{1}=data.timestamps{1}(750);
data.values{1}(750:end)=data.values{1}(750:end)+6;
