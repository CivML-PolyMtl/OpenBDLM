function [logpdf, Glogpdf, Hlogpdf, delta_grad] = logPosteriorPE(data, model, option, varargin)
% INPUTS:
...
if strcmp(option.optim_mode,'MAP')
    ...
    for i = 1:nb_param
        ...
        [logPrior_loop(i), GlogPrior_loop(i), HlogPrior_loop(i)]= logPriorDistr(model.parameterTR(idx), logpriorMu(idx), logpriorSig(idx),'distribution',logpriorName{idx});
         logPrior = logPrior + logPrior_loop(i);
    end
elseif strcmp(option.optim_mode,'MLE')
...
if getlogpdf
    ...
    [Gloglik, Hloglik, fail_gradHess, delta_grad] = gradHess(data, model, option,...
...
function [grad, hessian, fail_gradHess, delta_grad] = gradHess(data, model, option,...
...
while hessian_test
    ...
    [delta_grad, fail_delta_grad, log_lik_1, log_lik_2] = StepSizeOptimization(model, data, option,...
...
function [delta_grad, fail_delta_grad, log_lik_1, log_lik_2] = StepSizeOptimization(model, data, option,...
                                                                                    pOR, log_lik_0, delta_grad,...
                                                                                    param_idx_loop)
